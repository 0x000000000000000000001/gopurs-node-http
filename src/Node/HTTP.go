...
func createServerInternal(serverEmitter *Node_EventEmitter.EventEmitter) {
	...
	httpServer := &http.Server{
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if isUpgrade {
				...
				Node_EventEmitter.GopursUnsafeEmitFn4(gopurs_runtime.Box(serverEmitter), "upgrade", gopurs_runtime.Box(reqEmitter), gopurs_runtime.Box(socketEmitter), gopurs_runtime.Box[any](nil), nil)
			} else {
				reqEmitter := Node_EventEmitter.NewImpl(nil).(*Node_EventEmitter.EventEmitter)
				reqEmitter.Any = r
